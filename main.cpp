...
int main(int argc, char* argv[])
{
	...
	while (apprunning)
	{
		...
		for (size_t i = 0; i < GameObjects.size(); i++)
		{
			...
			{
				var += MeshComponent->DrawCube();
			}
			else
			{
				if (render)
				{
					PhysicsEngine->rigidbodies[i]->setActivationState(ACTIVE_TAG);
					var += MeshComponent->DrawSphere();
					++nbBall;
				}
				else
					PhysicsEngine->rigidbodies[i]->setActivationState(WANTS_DEACTIVATION);
			}
		}
		...
		ImGui::Checkbox("Enable/disable render balls", &render);
		ImGui::End();
